...
$act_query = "UPDATE forum_users set lastactive=now(), location='browsing outbox' where username='$login'";
mysql_query($act_query);


$query = "UPDATE friends set lastactive=now(), location='browsing outbox' where friendname='$login'";
mysql_query($query);
...
$query = "SELECT count(*) from phoenix_mail where author='$login' and read_state=0 and bound='out'";
...
if ($bwmode == "off") $subtite = "<img align=\"middle\" src=\"../scripts/phoenix.php?login=$login&amp;string=outbox($count);\" alt=\"outbox($count);\"/>";
else $subtite = "<b><big>outbox($count);</big></b>";
...
$query = "select * from phoenix_mail where author='$login' and bound='out' ORDER BY stamp DESC LIMIT $start, $max";
$result = mysql_query($query);
...
$row = mysql_fetch_array($result);
		...
		{
		if ($counter != $page) echo "<b><a href=\"./outbox.php?ses=$ses&amp;page=$counter\">$counter</a></b> ";
...
while ($row)
	...
	echo "<p class=\"breakforum\">$prefix<b><a href=\"./outmsg.php?ses=$ses&amp;mid=$mid\">$subject</a></b> to <b><a href=\"../profile.php?ses=$ses&amp;user=$userto&amp;pot=head\">$userto</a></b>$suffix</p>";
	$row = mysql_fetch_array($result);
	}
...
$hyfor <a href=\"./insert.php?ses=$ses&amp;act=delout\">empty outbox</a><br/>
$hyback <a href=\"./index.php?ses=$ses&amp;act=index\">messaging</a><br/>
$hyback <a href=\"../mainmenu.php?ses=$ses&amp;act=index\">main menu</a>$shortcuts</p></body></html>";
